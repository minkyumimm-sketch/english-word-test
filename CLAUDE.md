...
js/utils/              dom.js（DOMヘルパー）, validators.js（入力検証）, csv.js（CSVパース／書き出し、Ver2.8でstringify・buildFileNameを追加）
...
- 読み込んだ単語データ一覧のCSV書き出し（Ver2.8）は`utils/csv.js`に追加した`stringify(rows)`
  （既存の`parse()`の逆変換）と`buildFileName(materialName)`が中心。ダウンロード自体は
  `main.js`の`handleExportCsv()`が`Blob`＋一時的な`<a download>`要素のクリックで行う
  （新しいUIコンポーネントやモーダルは作らず、ブラウザ標準のダウンロード動作に委ねる）。
  新しいlocalStorageキーは増やしていない。`buildFileName()`は、Windowsで使用できない
  文字（`\ / : * ? " < > |`）の除去→空なら既定名「単語データ」にフォールバック、という
  順で安全なファイル名に変換してから`.csv`を付与する。詳細は`docs/DESIGN.md`の
  「12. 読み込んだ単語データ一覧のCSV書き出し」を参照。
- 「単語テスト名」（`state.getCurrentTestName()`、Ver2.8）は、CSV書き出しファイル名と
  「タイトル（任意）」欄の初期値の**両方**が使う、唯一の取得口。呼び出し側
  （`main.js`）は`state.getSourceFileName()`を直接使わないこと。中身は`sourceFileName`
  （10章・Ver2.2の教材名）から既知の拡張子（`.xlsx`/`.xlsm`/`.xls`/`.csv`）を除いた文字列
  で、拡張子除去そのものは`utils/csv.js`の`stripKnownExtension()`（拡張子リストの実体を
  持つ唯一の場所）を呼ぶだけ。教材名とテスト名は現時点では同じ値だが将来別概念になり
  得るため、両方の機能のコードが`sourceFileName`という実装詳細に直接依存しないよう
  あえてワンクッション挟んである（分離する場合は`getCurrentTestName()`の中身だけ
  差し替えればよく、呼び出し側は変更不要）。
  タイトル欄（`main.js`の`applyLoadedWords()`・`upgradeGoogleSheetTitle()`）は、教材の
  読込が成功するたび`formView.setTestTitle(state.getCurrentTestName())`で上書きする。
  タイトル欄を書き換えるコードはこの2箇所のみで、他のイベント（入力欄変更・生成予定
  プレビュー再計算等）では一切触れないため、「ユーザーが手動で書き換えたタイトルを
  途中で消してしまわない」という要件を、書き込みタイミングをこの2箇所だけに限定する
  ことで満たしている（同じ教材を再読込した場合も新しい値で上書きする、区別をしない
  シンプルな実装）。アプリ起動直後の復元フローでは、この直後に`restorePersistedSettings()`
  が前回保存したタイトルで上書きするため、最終的にはそちらが優先される（既存の設定復元
  順序に乗るだけで、この機能のための特別分岐は不要だった）。詳細は`docs/DESIGN.md`の
  「13. 「単語テスト名」とタイトル欄の自動入力」を参照。